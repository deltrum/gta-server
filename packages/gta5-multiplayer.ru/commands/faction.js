...
module.exports =
{
	...
	"ex" : (player, args) =>
	{
		...
		if(!API.radius(CONST.RANGE_PROXIMITY, mp.players.at(args[1]).position, player.position)) return player.call("alert", "error" , "Вы слишком далеко!"); 
	...
	"cuff" : (player, args) => 
	{
		...
		if(mp.players.at(parseInt(args[1])).cuff) {
			mp.players.broadcastInRange(player.position, CONST.RANGE_CHAT, "<font color='#c2a2da'><b>"+player.name + " снял(а) наручники с " + mp.players.at(args[1]).name +"</b></font>");
			...
			mp.players.broadcastInRange(player.position, CONST.RANGE_CHAT, "<font color='#c2a2da'><b>"+player.name + " надел(а) наручники на " + mp.players.at(args[1]).name +"</b></font>");
	...
	"n" : (player, args) =>
	{
		if(player.customData.member !== CONST.FACTION_LSNEWS) return player.call("alert", "error" , "Вы не работаете в LS NEWS");
	...
	"subsidy" : (player, args) => 
	{
		...
		if(player.customData.member !== CONST.FACTION_CITYHALL) return player.call("alert", "error" , "Вы не состоите в мэрии.");
		if(mp.players.at(parseInt(args[1])) === null) return player.call("alert", "error" , "Неверный ID игрока");
		...
		mp.players.broadcastInRange(player.position, CONST.RANGE_CHAT, "<font color='#c2a2da'><b>"+ player.name +" выдал(а) суббсидию " + mp.players.at(args[1]).name +"</b></font>");	
	},
	"rebank" : (player, args) => 
	{
	...
	"frisk" : (player, args) => 
	{
		...
		if(!API.radius(CONST.RANGE_PROXIMITY, mp.players.at(args[1]).position, player.position)) return player.call("alert", "error" , "Вы слишком далеко!"); 		
        mp.players.broadcastInRange(player.position, CONST.RANGE_CHAT, "<font color='#c2a2da'><b>"+player.name +" обыскал(а) " + mp.players.at(args[1]).name + "</b></font>");
		player.call("items", parseInt(args[1]), JSON.stringify(mp.players.at(args[1]).customData.item));
	},
	"free" : (player, args) =>
	{
		if(player.customData.member !== CONST.FACTION_CITYHALL || player.customData.rank !== 2) return player.call("alert", "error" , "Вы не работаете адвокатом.");
	...
	"heal" : (player, args) =>
	{
		if(player.customData.member !== CONST.FACTION_MOH) return player.call("alert", "error" , "Вы не работаете в MOH LS");
	...
	"gov": (player, args) =>
    {
		...
		if(!API.isGovernmentFaction(player.customData.member)) return player.call("alert", "error" , "Вы неуполномочены выходить в гос.новости.");
	...
	"demote": (player, args) =>
    {
		...
		if(player.customData.member !== CONST.FACTION_CITYHALL || player.customData.rank < game.faction[player.customData.member].rank.length) return player.call("alert", "error" , "Вы неуполномочены использовать эту команда.");
		if(mp.players.at(parseInt(args[1])) === null) return player.call("alert", "error" , "Неверный ID игрока");
		if(args[1] === player.id) return player.call("alert", "error" , "Вы указали свой ID");
		if(!API.isGovernmentFaction(mp.players.at(args[1]).customData.member)) return player.call("alert", "error" , "Не является гос.служищим");
		...
		if(parseInt(args[2]) === 0)
		{
			...
			mp.players.forEach(_player => { if(API.isGovernmentFaction(_player.customData.member)) _player.outputChatBox("<b><font color='#FF6347'><< " + game.faction[player.customData.member].rank[player.customData.rank-1] + " " + player.name + " уволил " + mp.players.at(args[1]).name +". >></font></b>"); });
			mp.players.at(args[1]).customData.member = 0;
			mp.players.at(args[1]).customData.rank = 0;
			mp.players.at(args[1]).customFunc.generate();			
		}
		else if(parseInt(args[2]) > mp.players.at(args[1]).customData.rank)
		{
			...
			mp.players.forEach(_player => { if(API.isGovernmentFaction(_player.customData.member)) _player.outputChatBox("<b><font color='#FF6347'><< " + game.faction[player.customData.member].rank[player.customData.rank-1] + " " + player.name + " повысил(а) " + mp.players.at(args[1]).name +". >></font></b>"); });
		...
		else if(parseInt(args[2]) < mp.players.at(args[1]).customData.rank)
		{
			...
			mp.players.forEach(_player => { if(API.isGovernmentFaction(_player.customData.member)) _player.outputChatBox("<b><font color='#FF6347'><< " + game.faction[player.customData.member].rank[player.customData.rank-1] + " " + player.name + " понизил(а) " + mp.players.at(args[1]).name +". >></font></b>"); });
			mp.players.at(args[1]).customData.rank = parseInt(args[2]);			
		}
    },	
	"showudost": (player, args) =>
    {
		...
		if(!API.isPoliceFaction(player.customData.member)) return player.call("alert", "error" , "Вы не работаете в полиции."); 		
		if(!API.radius(CONST.RANGE_PROXIMITY, mp.players.at(args[1]).position, player.position)) return player.call("alert", "error" , "Вы слишком далеко!"); 
        mp.players.broadcastInRange(player.position, CONST.RANGE_CHAT, "<font color='#c2a2da'><b>"+player.name + " показал(а) удостоверение " + mp.players.at(args[1]).name +"</b></font>");
	...
	"d": (player, args) =>
    {
		if(!API.isGovernmentFaction(player.customData.member)) return player.call("alert", "error" , "Вы не можете выходить в гос.депортамент.");
		if(player.customData.mute > 0) return player.call("alert", "warning" , "У вас молчанка! Осталось: " + player.customData.mute + " сек.");
	...
	"r": (player, args) =>
    {
		if(!API.isGovernmentFaction(player.customData.member)) return player.call("alert", "error" , "У вас нет рации.");
		if(player.customData.mute > 0) return player.call("alert", "warning" , "У вас молчанка! Осталось: " + player.customData.mute + " сек.");
	...
	"f": (player, args) =>
    {
		if(player.customData.member !== CONST.FACTION_LSNEWS || player.customData.member !== CONST.FACTION_DRIVING_SCHOOL) return player.call("alert", "error" , "У вас нет семьи.");
		if(player.customData.mute > 0) return player.call("alert", "warning" , "У вас молчанка! Осталось: " + player.customData.mute + " сек.");
	...
	"m": (player, args) =>
    {
		...
		mp.players.broadcastInRange(player.position, CONST.RANGE_MEGAPHONE, "<font color ='#ffff00'>[" + game.faction[player.customData.member].member + "] " + player.name + " " + text + "</font>");
	},
	"arrest": (player, args) =>
    {
		if(!API.isPoliceFaction(player.customData.member)) return player.call("alert", "error" , "Вы не работаете в полиции.");
		...
		_player.customData.jail = CONST.JAIL_TIME_MULTIPLIER * _player.customData.offense;
		_player.outputChatBox("<font color='red'><b>Вы были арестованы на: " + String(CONST.JAIL_TIME_MULTIPLIER * _player.customData.offense) + " секунд. Арестовал: "+ player.name + ".</b></font>");
		mp.players.forEach(__player => { if(API.isPoliceFaction(__player.customData.member)) __player.outputChatBox("<b><font color='#FF6347'><< Офицер " + player.name + "арестовал " + _player.name +". >></font></b>"); });	
