...
//---------------------------------------- [ RP-команды ] ----------------------------------------
	...
	"s": (player, args) =>
    {
		...
		args.shift();
		let text; if((text = API.striptags(args.join(" "), "<b><s><u><strong>")) === "") return player.call("alert", "information" , "Используйте: /s [text]");
	...
	"me": (player, args) =>
    {
		...
		args.shift();
		let text; if((text = API.striptags(args.join(" "), "<b><s><u><strong>")) === "") return player.call("alert", "information" , "Используйте: /me [text]");
		if(player.customFunc.testFloodText(text)) return;
	...
	"do": (player, args) =>
    {
		...
		args.shift();
		let text; if((text = API.striptags(args.join(" "), "<b><s><u><strong>")) === "") return player.call("alert", "information" , "Используйте: /do [text]");
		if(player.customFunc.testFloodText(text)) return;
	...
	"try": (player, args) =>
    {
		...
		args.shift();
		let text; if((text = API.striptags(args.join(" "), "<b><s><u><strong>")) === "") return player.call("alert", "information" , "Используйте: /try [text]");
		if(player.customFunc.testFloodText(text)) return;
	...
	"w": (player, args) =>
    {
		...
		args.shift();
		let text; if((text = API.striptags(args.join(" "), "<b><s><u><strong>")) === "") return player.call("alert", "information" , "Используйте: /w [text]");
		if(player.customFunc.testFloodText(text)) return;
	...
	"b": (player, args) =>
    {
		...
		args.shift();
		let text; if((text = API.striptags(args.join(" "), "<b><s><u><strong>")) === "") return player.call("alert", "information" , "Используйте: /b [text]");
		if(player.customFunc.testFloodText(text)) return;
	...
	"pay" : (player,args) =>
	{
		...
		if(args.length !== 3 || isNaN(parseInt(args[1])) || isNaN(money)) return player.call("alert", "information" , "Используйте: /pay [id] [money]");
		const _player = mp.players.at(parseInt(args[1]));
 		if(_player === null) return player.call("alert", "error" , "Неверный ID игрока");	
		if(!API.radius(CONST.RANGE_PROXIMITY, _player.position, player.position)) return player.call("alert", "error" , "Вы слишком далеко!"); 		
    ...
    "exit": (player, args) =>
    {
		  if(player.customData.enter_house >= 0) {
		    pool.query('SELECT * FROM houses WHERE id = ?', [player.customData.enter_house], function (error, results, fields) {
		    ...
		    pool.query('SELECT * FROM houses WHERE id = ?', [player.customData.enter_garage], function (error, results, fields) {
		      player.position = new mp.Vector3(parseFloat(results[0].pos_x),parseFloat(results[0].pos_y),parseFloat(results[0].pos_z));
		      player.dimension = 0;
		      player.customData.enter_limit = 1;
    ...
    "fixcars": (player, args) =>
    {
		  pool.query("SELECT * FROM houses WHERE owner = ?", [player.name], function(err1, results1) {
		    if(results1[0]) {
		      pool.query("SELECT * FROM cars WHERE person = ?", [player.name], function(err2, results2) {
		        if(results2) {
	 ...
	 "addhouse": (player, args) =>
	 {
	 	...
	 	const rare = args[1], coast = args[2], interior = parseInt(args[3]), garage = args[4], max_cars = parseInt(args[5]);
	 	if (isNaN(parseInt(rare)) || isNaN(parseInt(coast)) || isNaN(interior) || isNaN(parseInt(garage)) || isNaN(max_cars)) return player.call("alert", "error" , "Используйте: /addhouse [rare] [coast] [interior] [garage] [max_cars]"); 
	      if(parseInt(rare) >= 0 && parseInt(coast) >= 0 && parseInt(interior) >= 0 && parseInt(garage) >= 0 && max_cars >= 1) {
	        let get_id = 0;
	        let get_count = 0;
	        pool.query('SELECT COUNT(*) AS count FROM houses', [], function (error, results, fields) {
	          ...
	          var query = pool.query('INSERT INTO houses SET pos_x = ?, pos_y = ?, pos_z = ?, pos_heading = ?, rare = ?, coast = ?, interior = ?, garage = ?, garage_enter_pos_x = ?, garage_enter_pos_y = ?, garage_enter_pos_z = ?, garage_enter_pos_r = ?, max_cars_count = ?, max_cars_pos = ?', [player.position.x, player.position.y, player.position.z, player.heading, rare, coast, interior, garage, 0, 0, 0, 0, max_cars, max_cars_pos], function (error, results, fields) {
	            console.log(error);
	          });
	 ...
	 if(args.length !== 2 || isNaN(parseInt(houseid))) return player.call("alert", "error", "Используйте: /house [number]");
      if (parseFloat(houseid)) {
        pool.query('SELECT * FROM houses WHERE id = ?', [houseid], function (error, results, fields) {
          if(results[0]) {
            player.position = new mp.Vector3(parseFloat(results[0].pos_x),parseFloat(results[0].pos_y),parseFloat(results[0].pos_z));
          } else {
    ...
    "setgarage": (player, args) =>
    {
    	if(player.customData.admin < CONST.ADMIN_TECH) return player.call("alert", "error" , "У Вас недостаточно полномочий");
    	const house = args[1];
    	if(args.length !== 2 || isNaN(parseInt(house))) return player.call("alert", "error", "Используйте: /setgarage [number]");
        let get_house;
        pool.query('SELECT * FROM houses WHERE id = ?', [house], function (error, results, fields) {
          get_house = results[0];
          if(get_house) {
            var query = pool.query('UPDATE `houses` SET garage_enter_pos_x = ?, garage_enter_pos_y = ?, garage_enter_pos_z = ?, garage_enter_pos_r = ? WHERE id = ?', [player.position.x, player.position.y, player.position.z, player.heading, house], function (error, results, fields) {
            });
